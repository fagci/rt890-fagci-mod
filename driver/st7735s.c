...
void ST7735S_defineScrollArea(uint16_t x, uint16_t x2) {
  ...
  /* vsa: height of the vertical scrolling area in nr of line of the frame mem
     (not the display) from the vertical scrolling address. the first line
     appears immediately after the bottom most line of the top fixed area. */
  uint16_t vsa = x2 - x + 1;
  /* bfa: bottom fixed are in nr of lines from bottom of the frame memory and
   * display */
  uint16_t bfa = x + 1;
